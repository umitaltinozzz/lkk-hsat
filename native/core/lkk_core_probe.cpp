// Drift probe: the shared core with the ACCEPTED registry.
//
// Not a product engine. It exists so native/core/test_core_drift.py can
// compare the extracted core against the sealed Phase 4A binary even after
// the product engines moved to the Phase 4C search.
//
...
static int solve_main(int argc,char**argv){
  ...
  int seed=1; long db=5000; bool telemetry_only=false; double timeout_s=0; SelectorConfig sel;
  ...
  if(stop_after!="none"){ gate=fast_gate(cnf);
    if(gate.decision=="RUN_LKK"&&stop_after!="gate"){ benefit=benefit_gate(cnf,gate);
      if(benefit.decision=="RUN_LKK"){ rec=recover(cnf,db); recovery_ms=rec.elapsed; abort_reason=rec.status=="ABORT"?rec.reason:"";
        if(stop_after!="recovery"){ reg=registry(rec); registry_ms=reg.elapsed;
