#ifndef LKK_CORE_HPP
#define LKK_CORE_HPP
// The accepted LKK-HSAT 3.0 structural core, in one place.
//
// Until Phase 4C these functions were copied byte-for-byte into every engine
// binary, and the claim that the copies stayed identical lived only in a
// comment. They now have a single definition, and `native/core/test_core_drift`
// differential-tests a binary built from this header against the sealed
// Phase 4A binary so the claim is enforced rather than asserted.
//
// Nothing here may change without a new phase and a fresh correctness campaign:
// parse_dimacs, fast_gate, benefit_gate, recover, combinations, registry,
// flow_test and write_witness define the accepted semantics. `registry` is the
// accepted candidate search; Phase 4C supplies an indexed replacement in its own
// translation unit and must agree with this one under an unbounded budget.
...
static CNF parse_dimacs(const std::string &path) {
  ...
  while(std::getline(in,line)) {
    // '%' introduces the trailer SATLIB appends after the last clause.
    ...
    while(s>>lit) { if(lit) current.push_back(lit); else {
      // A lone terminator once the declared clauses have all been read is that
      // same trailer, which the locked solvers also tolerate. It is only
      // skipped when clauses are no longer owed: a genuine empty clause makes
      // the formula unsatisfiable and must never be dropped.
      if(current.empty() && expected>=0 && (int)out.clauses.size()>=expected) continue;
      canon(current); out.clauses.push_back(current); current.clear(); } }
  }
...
// `atleast` marks a box whose evidence establishes only the lower bound
// sum(vars) >= demand, without the matching upper bound. The cut argument uses
// nothing else, so such a box is as good as an exact-r one for a refutation;
// the flag exists so the witness can say which was proved and the checker can
// ask for the right evidence.
struct Box { int id,demand; std::vector<int> vars,evidence; bool atleast=false; };
