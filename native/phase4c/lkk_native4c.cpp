// Phase 4C engine: the accepted architecture with an indexed registry search.
//
// The structural semantics come from native/core/lkk_core.hpp. The only
// difference from the accepted engine is how the exact-r candidate search
// spends its budget:
//
//   * candidates are found through an inverted index from a variable pair to
//     the negative clauses containing it, instead of a full cross product, and
//   * a candidate's family is verified one clause at a time and each decided
//     group is remembered, instead of charging the whole family before testing
//     any of it.
//
// The acceptance test, candidate key, selection order and the meaning of a
// "check" are unchanged, so with an unbounded budget this returns exactly what
// lkk_core.hpp's registry() returns. phase4c/test_phase4c.py proves that
...
static int solve_main(int argc,char**argv){
  ...
  int seed=1; long db=5000,family_budget=300000; bool telemetry_only=false; double timeout_s=0; SelectorConfig sel;
  ...
  if(stop_after!="none"){ gate=fast_gate(cnf);
    if(gate.decision=="RUN_LKK"&&stop_after!="gate"){ benefit=benefit_gate(cnf,gate);
      if(benefit.decision=="RUN_LKK"){ rec=recover(cnf,db); recovery_ms=rec.elapsed; abort_reason=rec.status=="ABORT"?rec.reason:"";
        if(stop_after!="recovery"){ reg=registry_indexed(rec,family_budget); registry_ms=reg.elapsed;
